...
function launchWebScript(r) {
  return [
    ...
    // A dedicated browser profile makes `--app` open a NEW standalone instance
    // (an isolated window) rather than a tab in the already-running browser.
    '$winProfile = Join-Path $logDir "window-profile"',
    'if (-not (Test-Path $winProfile)) { New-Item -ItemType Directory -Path $winProfile -Force | Out-Null }',
    '$winArgs = @("--app=" + $url, "--user-data-dir=" + $winProfile)',
    'if ($browser) {',
    '  Start-Process -FilePath $browser -ArgumentList $winArgs',
    '} else {',
    '  Start-Process $url',
    '}',
  ].join('\n')
...
function ensureShortcutScript(r, launcherPath, iconPath, desktopName) {
  ...
  const lines = [
    ...
    `$log = Join-Path ${psQuote(r.logsDir)} 'dsh-shortcut.log'`,
    'function Log($m) { try { Add-Content -Path $log -Value ((Get-Date -Format "yyyy-MM-dd HH:mm:ss") + " " + $m) } catch {} }',
    `$launcher = ${psQuote(launcherPath)}`,
    `$desktopName = ${psQuote(name)}`,
    'try {',
    // Resolve the real Desktop, retrying briefly — OneDrive-redirected desktops
    // can be slow to mount right after boot.
    '  $desktop = $null',
    '  for ($i = 0; $i -lt 6 -and -not $desktop; $i++) {',
    '    $desktop = [Environment]::GetFolderPath("Desktop")',
    '    if (-not $desktop) { Start-Sleep -Seconds 2 }',
    '  }',
    '  if (-not $desktop) { $desktop = Join-Path $env:USERPROFILE "Desktop" }',
    '  if (-not $desktop) { throw "Desktop path could not be resolved" }',
    '  $lnk = Join-Path $desktop ($desktopName + ".lnk")',
  ...
  lines.push('  $sc.Description = $desktopName')
  lines.push('  $sc.Save()')
  lines.push('  Log("created shortcut: " + $lnk)')
  lines.push('} catch {')
  lines.push('  Log("shortcut creation failed: " + $_.Exception.Message)')
  lines.push('}')
  return lines.join('\n')
}
...
function bootstrapScripts(ctx, r, cfg, desktopName) {
  try {
    ...
    // app-window launcher. Failures are logged by the script itself.
    if (cfg.createShortcut !== false) {
      const iconPath = resolveBundledIcon()
      writeFileSync(shortcutPath, '\uFEFF' + ensureShortcutScript(r, launchPath, iconPath, desktopName), 'utf8')
