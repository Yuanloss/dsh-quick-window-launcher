...
/** Ask the running harness for the URL that carries this process's launch
 *  token (the same URL dsh web prints). Opening it mints the auth cookie in
 *  the app-window's isolated profile, so the window is authenticated like the
 *  user's normal browser instead of showing "authentication required". */
function fetchAuthUrl() {
  return new Promise((resolve) => {
    const rq = http.get(url + '/api/dsh-web-auth-url', (res) => {
      let body = ''
      res.on('data', (c) => { body += c })
      res.on('end', () => {
        try {
          const j = JSON.parse(body)
          resolve(j && j.ok && j.value && typeof j.value.url === 'string' ? j.value.url : '')
        } catch { resolve('') }
      })
    })
    rq.on('error', () => resolve(''))
    rq.setTimeout(2000, () => { rq.destroy(); resolve('') })
  })
}
...
async function main() {
  ...
  // Standalone app window: prefer a Chromium browser in --app mode. The URL
  // carries the launch token so the isolated window profile mints the auth
  // cookie (falls back to the plain URL when the route is unavailable).
  const openUrl = (await fetchAuthUrl()) || url
  log('opening window with url: ' + openUrl)
  ...
  if (CFG.platform === 'win32') {
    if (browser) {
      ...
      spawn(browser, ['--app=' + openUrl, '--user-data-dir=' + winProfile], { detached: true, stdio: 'ignore' }).unref()
      log('opened app window via ' + browser)
    } else {
      spawn('rundll32.exe', ['url.dll,FileProtocolHandler', openUrl], { stdio: 'ignore', windowsHide: true }).unref()
      log('opened default browser via rundll32')
    }
  } else if (CFG.platform === 'darwin') {
    spawn('open', [openUrl], { stdio: 'ignore' }).unref()
    log('opened default browser via open')
  } else {
    spawn('xdg-open', [openUrl], { stdio: 'ignore' }).unref()
...
export default {
  ...
  apply(ctx, config) {
    ...
    // The client-connection service knows this process's launch token (the
    // token dsh web prints in its URL). Its authenticatedUrl() returns the
    // URL that mints the browser cookie — the launcher uses it so the
    // isolated app-window profile gets authenticated exactly like the user's
    // normal browser (which already carries the cookie).
    ...
    const dshAuthUrlHandler = (req, res) => {
      const remote = req.socket?.remoteAddress ?? ''
      if (!LOOPBACK.has(remote)) {
        writeJson(res, 403, { ok: false, error: { code: 'forbidden', message: 'loopback-only' } })
        return
      ...
      let url = r.url
      try {
        if (connectionSvc && typeof connectionSvc.authenticatedUrl === 'function') url = connectionSvc.authenticatedUrl(r.url)
      } catch (e) { /* fall back to the plain URL */ }
      writeJson(res, 200, { ok: true, value: { url } })
    }
    ...
    ctx.effect(() => webServer.register({ kind: 'exact', path: '/api/dsh-web-auth-url', handler: dshAuthUrlHandler }))
  },
}
