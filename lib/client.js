...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + STYLE_TAG + '"]') === null) {
      ...
      tag.textContent = [
        // Base action: a compact icon-only circle (rail / collapsed sidebar).
        '.dsh-cl-btn { display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; padding: 0; cursor: pointer; border: 1px solid transparent; border-radius: 50%; background: transparent; transition: background-color .12s ease, color .12s ease, border-color .12s ease, box-shadow .12s ease; }',
        '.dsh-cl-btn .dsh-cl-ico { font-size: 16px; line-height: 1; }',
        // Label is hidden unless the action is in the wide (expanded) state.
        '.dsh-cl-btn .dsh-cl-label { display: none; font-size: 12px; white-space: nowrap; }',
        // Wide state: a pill with icon + label, vertically centered.
        '.dsh-cl-btn.dsh-cl-wide { width: auto; height: 36px; padding: 0 10px; gap: 6px; border-radius: 8px; }',
        '.dsh-cl-btn.dsh-cl-wide .dsh-cl-label { display: inline; }',
    ...
    function ShutdownAction(props) {
      ...
      var cls = 'dsh-cl-btn dsh-cl-shutdown' + (wide ? ' dsh-cl-wide' : '')
      return React.createElement(
        'button',
        { type: 'button', className: cls, title: title, 'aria-label': title, disabled: pending, onClick: onClick },
        React.createElement('span', { className: 'dsh-cl-ico' }, pending ? '…' : '⏻'),
        React.createElement('span', { className: 'dsh-cl-label' }, '关闭'),
      )
    }
    ...
    function RestartAction(props) {
      ...
      var cls = 'dsh-cl-btn dsh-cl-restart' + (wide ? ' dsh-cl-wide' : '')
      return React.createElement(
        'button',
        { type: 'button', className: cls, title: title, 'aria-label': title, disabled: pending, onClick: onClick },
        React.createElement('span', { className: 'dsh-cl-ico' }, pending ? '…' : '⟳'),
        React.createElement('span', { className: 'dsh-cl-label' }, '重启'),
      )
    }
    ...
    function DshUpdateAction(props) {
      ...
      var cls = 'dsh-cl-btn dsh-cl-update' + (wide ? ' dsh-cl-wide' : '')
      return React.createElement(
        'button',
        { type: 'button', className: cls, title: title, 'aria-label': title, disabled: pending, onClick: onClick },
        React.createElement('span', { className: 'dsh-cl-ico' }, pending ? '…' : '⬆'),
        React.createElement('span', { className: 'dsh-cl-label' }, 'DSH 更新'),
      )
    }
