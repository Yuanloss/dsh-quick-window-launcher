...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Build one segment of the footer power-control pill (24px squircle).
     * Icon-only on purpose: the footer action line is shared with cost-panel
     * text and other plugins' buttons, so its free width is unpredictable
     * across installs — labeled pills clipped the trailing button at default
     * sidebar width. A fixed trio of compact segments always fits; each
     * carries a `title` tooltip and a distinct accent color.
     *
     * Resting state is a faint translucent tile; hover tints the tile with
     * the accent color, adds a soft glow ring and a 1px lift, so the three
     * actions read as one segmented control. Inline styles only: the
     ...
     * @param {object} o - { color, ico, title, pending, onClick, hover, setHover, circle }.
    ...
    function roundBtn(o) {
      ...
      var style = {
        ...
        borderRadius: o.circle ? '50%' : '7px',
        background: hovered ? hexA(o.color, 0.16) : 'rgba(255,255,255,0.04)',
        cursor: o.pending ? 'wait' : 'pointer',
        color: hovered ? o.color : hexA(o.color, 0.85),
        transition: 'background .15s ease, color .15s ease, box-shadow .15s ease, transform .15s ease',
      }
      if (hovered) {
        style.boxShadow = '0 0 0 3px ' + hexA(o.color, 0.18) + ', 0 2px 8px ' + hexA(o.color, 0.35)
        style.transform = 'translateY(-1px)'
      }
      if (o.pending) { style.opacity = 0.55 }
    ...
    // there.
    var BTN_ICO = { fontSize: '13px', lineHeight: 1, fontWeight: 600 }
    var BTN_COLORS = { shutdown: '#ff5f57', restart: '#4da3ff', update: '#ffb03a' }

    /** Shutdown button: confirm + POST /api/shutdown. (Rail mode renders it
     *  as a standalone circular power key via the `circle` prop.) */
    function ShutdownAction(props) {
      ...
      return roundBtn({ color: BTN_COLORS.shutdown, ico: pending ? '…' : '⏻', title: title, pending: pending, onClick: onClick, hover: hover, setHover: setHover, circle: !!(props && props.circle) })
    }
          ...
          .then(function (j) {
            ...
            if (j && j.ok && j.value) {
              ...
              if (v.updateAvailable) {
                var cmd = 'npm install -g @deepseek-ai/dsh@latest'
                var copied = false
                try { if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(cmd); copied = true } } catch (e) { /* ignore */ }
                msg = '发现 DSH 新版本！\n\n当前版本：v' + cur + '\n最新版本：v' + latest + '\n\n升级命令' + (copied ? '（已复制到剪贴板）' : '') + '：\n' + cmd + '\n\n更新完成后点击「重启」按钮生效。'
                window.alert(msg)
                var go = window.confirm('要在新的终端窗口中执行更新吗？\n\n' + cmd + '\n\n会打开一个可见终端（可能要求管理员权限），请等待它完成。')
                if (!go) return
                setPending(true)
                fetch('/api/dsh-update-run', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })
                  .then(function (r) { return r.json() })
                  .then(function (j2) {
                    setPending(false)
                    if (j2 && j2.ok) {
                      if (j2.value && j2.value.mode === 'terminal') {
                        window.alert('已打开终端执行更新。\n\n请等待 npm 完成（如需管理员权限会弹出提示，按提示操作即可）。\n完成后关闭该窗口，再点击「重启」按钮让新版本生效。')
                      } else {
                        window.alert('更新已在后台启动。\n\n日志文件：\n' + ((j2.value && j2.value.logPath) || '（未知）') + '\n完成后点击「重启」按钮生效。')
                      }
                    } else {
                      window.alert('启动更新失败：' + ((j2 && j2.error && j2.error.message) || '未知错误'))
                    }
                  })
                  .catch(function () { setPending(false); window.alert('启动更新失败：无法连接 DSH 服务。') })
              } else if (cur === 'unknown') {
                msg = '无法确定当前 DSH 版本。\nnpm 最新版本：v' + latest
              } else {
     ...
     *
     * Adaptive density: the collapsed rail physically fits one 24px circle,
     * so it shows only the red shutdown power key; the expanded sidebar gets
     * the full glass pill of three 24px squircle segments (~80px), color-coded
     * with `title` tooltips. Inline styles only; zero other CSS touched.
     */
    function QuickActions(props) {
      var wide = !!(props && props.wide)
      if (!wide) return React.createElement(ShutdownAction, { circle: true })
      return React.createElement(
        'div',
        {
          style: {
