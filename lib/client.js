...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + STYLE_TAG + '"]') === null) {
      ...
      tag.textContent = [
        // Base action: a compact icon-only circle. Fixed size + no flex grow so
        // the slot's flex layout cannot stretch it into a bar.
        '.dsh-cl-btn { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; min-width: 30px; max-width: 30px; flex: 0 0 auto; align-self: center; padding: 0; cursor: pointer; border: 1px solid transparent; border-radius: 50%; background: transparent; transition: background-color .12s ease, color .12s ease, border-color .12s ease, box-shadow .12s ease; }',
