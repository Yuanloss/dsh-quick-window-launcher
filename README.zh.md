...
所有数据都在运行时从当前 harness 推导——Node 二进制、dsh 入口脚本、工作目录、监听端口，以及 `~/.dsh/logs`。**代码里没有任何硬编码的机器路径、用户名或安装位置**，因此可以安全发布，任何人安装即可用。
...
```powershell
dsh plugin --profile web add <你的用户名>/dsh-quick-window-launcher
```
